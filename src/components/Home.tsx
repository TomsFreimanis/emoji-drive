...
import { Zap, ShoppingBag, Sparkles, Coins, ArrowUpCircle, X, Lock, Crosshair, ChevronRight, Swords, BarChart3, UserPlus, CheckCircle, AlertCircle, Trophy, Skull, Briefcase } from 'lucide-react';
...
export const Home: React.FC<HomeProps> = ({ 
  ...
  const renderReferralModal = () => (
    <div className="fixed inset-0 z-[60] bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-200">
        <div className="bg-slate-900 w-full max-w-sm rounded-3xl border border-slate-700 p-6 shadow-2xl relative ring-1 ring-white/10 text-center">
            <button onClick={() => setShowReferralModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white p-2">
              <X className="w-6 h-6" />
            </button>
            
            <UserPlus className="w-16 h-16 text-green-400 mx-auto mb-4" />
            <h2 className="text-2xl font-display text-white mb-2">INVITE A FRIEND</h2>
            <p className="text-slate-400 text-sm mb-6">
                Send a link to your squad. If they join, you get a massive gold bonus to kickstart your collection!
            </p>

            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex items-center justify-between mb-6">
                <code className="text-slate-400 text-xs">emojioverdrive.com/join/u/123</code>
                <Button size="sm" variant="secondary" className="text-[10px] h-8 px-2">COPY</Button>
            </div>

            {referralClaimed ? (
                 <Button variant="secondary" disabled className="w-full bg-green-900/30 text-green-400 border-green-800">
                     <CheckCircle className="w-4 h-4 mr-2" /> REWARD CLAIMED
                 </Button>
            ) : (
                <Button variant="gold" size="lg" className="w-full" onClick={() => { onReferral(); setShowReferralModal(false); }}>
  ...
  const renderShop = () => (
    <div className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-200">
      <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-slate-700 p-6 shadow-2xl relative ring-1 ring-white/10">
        <button onClick={() => setShowShop(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white p-2">
          <X className="w-6 h-6" />
        </button>
        ...
        <div className="flex items-center gap-3 mb-8 pb-4 border-b border-slate-800">
          ...
          <div>
             <h2 className="text-2xl font-display text-white leading-none">ITEM SHOP</h2>
             <p className="text-slate-500 text-xs uppercase tracking-widest">Permanent Upgrades</p>
          </div>
          <div className="ml-auto bg-black/30 px-3 py-1 rounded-full border border-slate-700 flex items-center gap-2">
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 overflow-hidden font-sans relative">
       {/* Background Polish */}
      ...
      <div className="flex justify-between items-center p-4 z-10 relative">
        <div className="flex items-center gap-2">
           ...
           <div className="leading-tight">
             ...
             <span className="text-yellow-400 font-display text-sm tracking-widest block -mt-1">OVERDRIVE</span>
           </div>
        </div>
        <div className="flex gap-2 items-center">
           <button 
             onClick={() => { playSound('ui'); setShowReferralModal(true); }}
           ...
           >
             ...
             {!referralClaimed && <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full animate-ping" />}
             {!referralClaimed && <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full" />}
           </button>

           <button 
             onClick={() => { playSound('ui'); onLeaderboard(); }}
           ...
           <button 
             onClick={() => { playSound('ui'); setShowShop(true); }}
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar relative z-10 pb-40">
        <div className="px-4 flex flex-col gap-6 pt-2">
          ...
          <div className="w-full">
            <div className="bg-slate-900/60 backdrop-blur-sm rounded-2xl p-4 border border-slate-800 shadow-xl">
              <div className="flex items-center justify-between mb-4">
                 <h3 className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Mission Select</h3>
              ...
              {/* Map Nodes - SCROLLABLE CONTAINER */}
              <div className="relative w-full overflow-x-auto pb-4 hide-scrollbar">
                  <div className="flex items-center gap-6 px-2 py-2 min-w-max">
                     {/* Connecting Line */}
                     <div className="absolute top-1/2 left-4 right-4 h-1 bg-slate-800 -z-0 transform -translate-y-1/2 rounded-full"></div>
                     ...
                     {GAME_ZONES.map((zone) => {
                       ...
                       return (
                         <button 
                           key={zone.id}
                           onClick={() => { if (isUnlocked) { playSound('ui'); onSelectZone(zone); }}}
                           disabled={!isUnlocked}
                           className={`relative z-10 transition-all duration-300 group flex flex-col items-center gap-2
                             ${isSelected ? 'scale-110' : 'scale-100'}
                           `}
                         >
                            <div className={`w-14 h-14 rounded-2xl border-2 flex items-center justify-center text-2xl shadow-lg transition-all relative overflow-hidden
                               ${isSelected ? 'border-yellow-400 bg-slate-800 shadow-yellow-500/20' : 
                                 isUnlocked ? 'border-indigo-500/50 bg-slate-800' : 'border-slate-800 bg-slate-900 opacity-40'}
                            `}>
                               {isSelected && <div className="absolute inset-0 bg-yellow-400/10 animate-pulse" />}
                               {isUnlocked ? zone.icon : <Lock className="w-5 h-5 text-slate-600" />}
                            </div>
                         </button>
                       );
                     })}
              ...
              <div className="mt-4 flex flex-col gap-3">
                 <div className="flex items-center gap-4 bg-black/20 rounded-xl p-3 border border-white/5">
                    ...
                    <div className="flex justify-between items-center">
                        <h3 className="text-white font-bold text-sm tracking-wide">{currentZone.name}</h3>
                        <div className="flex gap-0.5">
                            {Array.from({length: 16}).map((_, i) => (
                            <div key={i} className={`w-1 h-3 rounded-full ${i < currentZone.difficulty ? 'bg-red-500' : 'bg-slate-800'}`} />
                            ))}
                        </div>
                    </div>
                    <p className="text-slate-400 text-[11px] mt-0.5 leading-tight">{currentZone.description}</p>
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-4 gap-2">
                    {(['EASY', 'NORMAL', 'HARD', 'EXTREME'] as Difficulty[]).map((d) => (
                        <button
                           key={d}
                           onClick={() => { playSound('ui'); onSelectDifficulty(d); }}
                           ...
                           `}
                        >
                           <div className={DIFFICULTY_TIERS[d].color}>{DIFFICULTY_TIERS[d].label}</div>
                           <div className="text-[9px] mt-1 text-slate-500 opacity-80">x{DIFFICULTY_TIERS[d].goldMult} Gold</div>
                        </button>
                    ))}
                 </div>
              </div>
          ...
          {/* Hero Showcase */}
          <div className="flex flex-col items-center relative py-2 animate-in zoom-in duration-500">
             
             <div className="text-center mb-4">
                <h2 className="text-3xl font-display text-white drop-shadow-lg">{selectedFighter.name}</h2>
                <div className="flex items-center justify-center gap-2 mt-1">
                   <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-900 px-2 py-0.5 rounded border border-slate-800">
                     {weaponInfo.name}
                   </span>
                   <span className="text-[10px] font-bold uppercase tracking-widest bg-black/50 px-2 py-0.5 rounded border border-white/10" style={{ color: rarityInfo.color }}>
             ...
             <div className="relative z-10 mb-6">
               <div className={`absolute inset-0 blur-[60px] rounded-full animate-pulse opacity-40`} style={{ backgroundColor: rarityInfo.color }}></div>
               <div className="w-40 h-40 relative transform transition-transform hover:scale-105 duration-300 group">
                  {selectedFighter.avatarImage ? (
                    ...
                    <div className={`w-full h-full flex items-center justify-center text-[7rem] drop-shadow-2xl bg-slate-800 rounded-3xl border-4 rotate-3 group-hover:rotate-0 transition-all ${rarityInfo.border}`}>
                  ...
                  <div className="absolute -bottom-3 -right-3 bg-slate-800 p-2 rounded-full border border-slate-700 shadow-lg">
                     {selectedFighter.weapon === 'BLASTER' && <Crosshair className="w-4 h-4 text-yellow-400"/>}
                     {selectedFighter.weapon === 'SNIPER' && <Crosshair className="w-4 h-4 text-blue-400"/>}
                     {selectedFighter.weapon === 'SHOTGUN' && <Crosshair className="w-4 h-4 text-red-400"/>}
                     {selectedFighter.weapon === 'RAPID' && <Zap className="w-4 h-4 text-green-400"/>}
                     {selectedFighter.weapon === 'HOMING' && <Sparkles className="w-4 h-4 text-purple-400"/>}
                  </div>
               </div>
             </div>
             ...
             <div className="grid grid-cols-2 gap-3 w-full max-w-sm">
                <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/50 backdrop-blur-sm">
                   <div className="space-y-2">
                      <div>
                        <div className="flex justify-between text-[10px] text-slate-400 font-bold uppercase mb-1">Power</div>
                        <div className="flex gap-0.5 h-1.5">
                          {Array.from({length: 10}).map((_, i) => (
                             <div key={i} className={`flex-1 rounded-full ${i < selectedFighter.baseStats.power ? 'bg-red-500' : 'bg-slate-700'}`} />
                          ))}
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[10px] text-slate-400 font-bold uppercase mb-1">Speed</div>
                        <div className="flex gap-0.5 h-1.5">
                          {Array.from({length: 10}).map((_, i) => (
                             <div key={i} className={`flex-1 rounded-full ${i < selectedFighter.baseStats.speed ? 'bg-blue-500' : 'bg-slate-700'}`} />
                          ))}
                        </div>
                      </div>
                   </div>
                ...
                <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/50 backdrop-blur-sm flex flex-col justify-center">
                   <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Special Ability</p>
                   <p className="text-white font-display tracking-wide">{selectedFighter.ability}</p>
                   <p className="text-[10px] text-slate-500 leading-tight mt-1 italic">"{selectedFighter.description}"</p>
                </div>
             </div>
          </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-slate-950/90 backdrop-blur-lg border-t border-white/5 p-4 pb-6 flex flex-col gap-4 z-30 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            ...
            <div className="flex gap-3 min-w-max px-1">
              {fighters.map(f => {
                 ...
                 return (
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded-2xl border-2 transition-all overflow-hidden relative group flex flex-col items-center justify-center
                        ${f.unlocked 
                    ...
                    {f.unlocked && (
                        <div className={`absolute bottom-0 left-0 right-0 h-1 ${rInfo.color.replace('text', 'bg')}`}></div>
                    )}
                    </button>
                 );
              })}
              ...
              {/* Hybrid Button */}
              <button 
                onClick={() => { playSound('ui'); onHybrid(); }}
                className="flex-shrink-0 w-16 h-16 rounded-2xl border-2 border-dashed border-slate-700 bg-slate-900/50 flex flex-col items-center justify-center gap-1 hover:border-purple-500 hover:text-purple-400 text-slate-500 transition-colors group"
              >
         ...
         {/* Main Action Button */}
         {isZoneLocked ? (
            <Button variant="secondary" className="w-full opacity-50 py-4" disabled>
              <Lock className="w-4 h-4 mr-2" /> COMPLETE ZONE {unlockedZones.length} FIRST
            </Button>
         ) : (
            <Button variant="primary" size="lg" className="w-full shadow-xl shadow-indigo-500/30 py-4 text-xl group" onClick={() => { playSound('ui'); onPlay(); }}>
              <div className="flex flex-col items-center leading-none">
                  ...
                  <span className={`text-[10px] uppercase tracking-widest mt-1 ${diffInfo.color}`}>
